...
export class WaveAmplifier extends Component {
  ...
  private messageText!: Phaser.GameObjects.Text; // Reference to the scene's message text for cooldown feedback

  constructor(enemiesGroup: Phaser.GameObjects.Group, messageText: Phaser.GameObjects.Text) {
    super();
  ...
  private activateWave(): void {
    ...
    this.gameObject.scene.tweens.add({
      targets: graphics,
      scale: 4, // Scale to reach desired radius
      alpha: 0,
      duration: 1000,
      ease: 'Quad.easeOut',
      onUpdate: (_tween, target) => {
        ...
        graphics.fillCircle(0, 0, (this.gameObject.width / 2) * target.scale);
        graphics.strokeCircle(0, 0, (this.gameObject.width / 2) * target.scale);
      },
      onComplete: () => {
        graphics.destroy();
      }
    ...
    // Cooldown feedback
    this.messageText.setText('Wave Amplifier on Cooldown!').setColor('#ff0000').setVisible(true);
    this.gameObject.scene.time.delayedCall(1500, () => this.messageText.setVisible(false));
  }
}
