...
export class WinScene extends Phaser.Scene {
    ...
    create(): void {
        ...
        this.add.text(WIDTH / 2, 200, 'CONGRATULATIONS!', {
            font: '80px',
            color: AppColors.UI_ACCENT,
            align: 'center'
        }).setOrigin(0.5);

        this.add.text(WIDTH / 2, 400, 'You have successfully defended the quantum realm.', {
            font: '32px',
            color: AppColors.UI_TEXT,
            align: 'center'
        }).setOrigin(0.5);

        const backButton = this.add.text(WIDTH / 2, 600, 'BACK TO MENU', {
            font: '36px',
            color: AppColors.UI_TEXT,
            padding: { x: 20, y: 10 }
