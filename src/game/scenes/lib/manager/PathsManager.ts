...
export class PathsManager extends Manager {
    ...
    setup() {
        ...
        for (let pathsKey in this.paths) {
            this.pathGraphics.lineStyle(0.5, AppColors.PATH_LINE, 0.30); // Use color constant
            ...
            this.pathGraphics.fillStyle(AppColors.PATH_START, 1); // Use color constant
            this.pathGraphics.fillCircle(startPoint.x, startPoint.y, 30);
            const endPoint = this.paths[pathsKey].getEndPoint();
            this.pathGraphics.fillStyle(AppColors.PATH_END, 1); // Use color constant
